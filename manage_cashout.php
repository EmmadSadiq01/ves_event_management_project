...
if(isset($_GET['id'])){
	$qry = $conn->query("SELECT * FROM cashout where id = ".$_GET['id'])->fetch_array();
	foreach($qry as $k => $v){
		$$k = $v;
	}
}
...
<div class="container-fluid">
	<form id='employee_frm'>
		...
		<div class="form-group">
			...
			<textarea name="providby" required="required" cols="30" rows="2" class="form-control"><?php echo isset($providby) ? $providby : "" ?></textarea>
		</div>
	</form>
</div>
<script>
	...
	$(document).ready(function(){
		...
		$('#employee_frm').submit(function(e){
			...
			$.ajax({
				...
				url:'ajax.php?action=save_cashout',
				method:"POST",
				data:$(this).serialize(),
				error:err=>console.log(),
				success:function(resp){
						if(resp == 1){
							alert_toast("Cash Out's data successfully saved","success");
								setTimeout(function(){
								location.reload();
