...
<?php
 ...
 $year = $_POST['year'];
 $sql = "SELECT bookings.eventDate, bookings.bookingAmount as 'bookamount', bookingtarget.target_price as 'targetamount' from bookings Right JOIN bookingtarget ON bookings.eventDate = bookingtarget.target_date WHERE bookings.hall_code = '$hallno' AND bookingtarget.hall_code = '$hallno' AND bookings.eventShift='evening' AND bookings.hallportion='a' AND bookingtarget.selectShift='evening' AND bookingtarget.selectHall='a' GROUP BY bookings.eventDate, bookingtarget.target_date HAVING bookings.eventDate like '%".$year."-__%' AND bookingtarget.target_date like '%".$year."-__%'";
