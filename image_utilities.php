...
<?php
		$pay = $conn->query("SELECT * FROM utilities where id = ".$_GET['id'])->fetch_array();
?>
<!DOCTYPE html>
<html>
<head>
			...
			<div class="col-lg-12">
				...
				<div class="card">
					...
					<?php if($_SESSION['login_type'] == 2): ?>
						<div class="row">
							<div class="col-md-12">
							<form method="post" enctype="multipart/form-data" action="utility_upload.php">
						...
						<table id="table" class="table table-bordered table-striped">
							...
							<tbody>
								<?php
									$images=$conn->query("SELECT * FROM utility_img where recept_no=".$pay['id']) or die(mysqli_error());
									while($row=$images->fetch_array()){
								?>
								<tr>
									<td>
                                        <div class="show-image">
                                            <image id="myImg" src="assets/utility_images/<?php echo $row["image_name"];?>" height="100" width="100"
                                            style="cursor:pointer" onclick="onClick(this)" class="modal-hover-opacity">
                                        </div>
                                    </td>
...
<!-- partial -->
  <script src="./assets/forimgcss/script.js"></script>
	...
	function remove_utility_image(id){
			start_load()
			$.ajax({
				url:'ajax.php?action=remove_utility_image',
				method:"POST",
				data:{id:id},
